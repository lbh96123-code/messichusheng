...
let i = 0; const send = () => { if (i >= steps.length) return setTimeout(() => process.exit(0), 40000); const [n, mk] = steps[i++]; console.log(`=== ${i}. ${n}`); const img = mk(); const buf = img.data.buffer.slice(0);
...
w.on("message", m => { if (m.type === "ready") { w.postMessage({ type: "display", w: 2560, h: 1440 }); return send(); }
  ...
  if (m.type === "state") setTimeout(send, m.idle ? 50 : 25000); });
