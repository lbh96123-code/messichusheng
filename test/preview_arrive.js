...
let i = 0; const send = () => { if (i >= steps.length) return setTimeout(() => process.exit(0), 15000); const [n, mk] = steps[i++]; console.log(`=== ${i}. ${n}`); const img = mk(); const buf = img.data.buffer.slice(0);
  w.postMessage({ type: "frame", w: img.w, h: img.h, buf, bgra: false, full: true, all: false, arrive: true }, [buf]); };
w.on("message", m => { if (m.type === "ready") { w.postMessage({ type: "display", w: 2560, h: 1440 }); return send(); }
  ...
  if (m.type === "advice" && m.stage === "done") console.log("  >>> done pre=" + m.pre + " arrive=" + JSON.stringify(m.arrive && { n: m.arrive.n, none: m.arrive.none, top: m.arrive.top.slice(0, 4).map(r => [r.name, r.alive, r.first]) }));
  if (m.type === "advice" && m.stage === "top") console.log(`  >>> 显示 ${m.side}${m.seat % 5 + 1} pre=${m.pre} 我方 ${(100 * m.base).toFixed(1)}%`);
  if (m.type === "state") setTimeout(send, m.idle ? 50 : 12000); });
